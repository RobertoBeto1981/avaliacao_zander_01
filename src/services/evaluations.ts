...
export const redistributeProfessorStudents = async (professorId: string) => {
  const { data, error } = await supabase.rpc('redistribute_professor_students', {
  ...
  const result = (data || {}) as {
    moved_count?: number
    failed_names?: string[]
    failed_shifts?: string[]
  }

  window.dispatchEvent(new CustomEvent('avaliacao_updated'))

  return {
    updated: result.moved_count || 0,
    failedNames: result.failed_names || [],
    failedShifts: result.failed_shifts || [],
  }
}
