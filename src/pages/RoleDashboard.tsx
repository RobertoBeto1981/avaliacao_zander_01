...
export default function RoleDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Mantém a regra original para este painel: exibe apenas os alunos atrelados ao avaliador, ou todos se for coordenador.
      setEvaluations(data.filter((ev: any) => ev.avaliador_id === profileId || isCoordenador))
    } catch (e: any) {
      toast({ variant: 'destructive', title: 'Erro', description: e.message })
    } finally {
      setInitialLoading(false)
    }
  }, [profileId, isCoordenador, toast])
    ...
    <div className="container mx-auto py-8 animate-fade-in-up">
      ...
      <Tabs defaultValue="painel" className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-8 h-12">
          <TabsTrigger value="painel" className="text-base">
            Painel de Atendimentos
          </TabsTrigger>
          <TabsTrigger value="comunicados" className="text-base">
            Comunicado Interno
          </TabsTrigger>
        </TabsList>
          ...
          {lateEvals.length > 0 && (
            ...
            >
              ...
              <AlertTitle className="text-lg font-bold">
                Atenção: Avaliações/Treinos Atrasados!
              </AlertTitle>
              <AlertDescription className="text-base">
                O sistema registra <strong>{lateEvals.length}</strong>{' '}
                {lateEvals.length === 1 ? 'avaliação' : 'avaliações'} com prazo de conclusão
