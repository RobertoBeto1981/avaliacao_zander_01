...
// Table: internal_chats
//   FOREIGN KEY internal_chats_avaliacao_id_fkey: FOREIGN KEY (avaliacao_id) REFERENCES avaliacoes(id) ON DELETE SET NULL
//   PRIMARY KEY internal_chats_pkey: PRIMARY KEY (id)
//   FOREIGN KEY internal_chats_receiver_id_fkey: FOREIGN KEY (receiver_id) REFERENCES users(id) ON DELETE CASCADE
//   FOREIGN KEY internal_chats_sender_id_fkey: FOREIGN KEY (sender_id) REFERENCES users(id) ON DELETE CASCADE
// Table: links_avaliacao
...
//     USING: ((EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('professor'::text = ANY (users.roles))))) AND ((professor_id = auth.uid()) OR ((is_pre_avaliacao = true) AND (professor_id IS NULL))))
...
//     USING: ((auth.uid() = sender_id) OR (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('coordenador'::text = ANY (users.roles))))) OR (EXISTS ( SELECT 1    FROM notifications   WHERE ((notifications.bulk_message_id = bulk_messages.id) AND (notifications.user_id = auth.uid())))))
// Table: evaluations
...
//     USING: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('avaliador'::text = ANY (users.roles)))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('avaliador'::text = ANY (users.roles)))))
//   Policy "Coordinators can manage links" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('coordenador'::text = ANY (users.roles)))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('coordenador'::text = ANY (users.roles)))))
//   Policy "Coordinators can view all links" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND ('coordenador'::text = ANY (users.roles)))))
//   Policy "Fisio and Nutri can manage links" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND (users.roles && ARRAY['fisioterapeuta'::text, 'nutricionista'::text]))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND (users.roles && ARRAY['fisioterapeuta'::text, 'nutricionista'::text]))))
//   Policy "Fisio and Nutri can view all links" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM users   WHERE ((users.id = auth.uid()) AND (users.roles && ARRAY['fisioterapeuta'::text, 'nutricionista'::text]))))
//   Policy "Professors can manage links of assigned avaliacoes" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM avaliacoes a   WHERE ((a.id = links_avaliacao.avaliacao_id) AND ((a.professor_id = auth.uid()) OR ((a.is_pre_avaliacao = true) AND (a.professor_id IS NULL))))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM avaliacoes a   WHERE ((a.id = links_avaliacao.avaliacao_id) AND ((a.professor_id = auth.uid()) OR ((a.is_pre_avaliacao = true) AND (a.professor_id IS NULL))))))
...
//   $function$
//
// FUNCTION cleanup_old_video_history()
...
//   $function$
//
// FUNCTION import_aluno_csv_safely(text, text, text, uuid)
...
//     $function$
//
// FUNCTION mark_chat_messages_as_read(text, text, uuid, text)
//   CREATE OR REPLACE FUNCTION public.mark_chat_messages_as_read(p_contact_id text, p_contact_type text, p_user_id uuid, p_contact_name text DEFAULT NULL::text)
//    RETURNS void
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
...
//   $function$
//
// FUNCTION upsert_aluno_dialog(text, text, text, uuid)
...
// --- TRIGGERS ---
// Table: avaliacao_acompanhamentos
//   on_acompanhamento_added_notify: CREATE TRIGGER on_acompanhamento_added_notify AFTER INSERT ON public.avaliacao_acompanhamentos FOR EACH ROW EXECUTE FUNCTION notify_professor_on_acompanhamento()
...
// Table: videos_agendados
//   trg_cleanup_old_video_history: CREATE TRIGGER trg_cleanup_old_video_history AFTER INSERT OR UPDATE ON public.videos_agendados FOR EACH STATEMENT EXECUTE FUNCTION cleanup_old_video_history()
