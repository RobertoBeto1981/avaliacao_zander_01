DO $
BEGIN
  -- Re-create policies on bulk_messages to allow ANY active user to send and read their own messages
  DROP POLICY IF EXISTS "Coordinators can insert bulk messages" ON public.bulk_messages;
  DROP POLICY IF EXISTS "Users can insert bulk messages" ON public.bulk_messages;
  
  CREATE POLICY "Users can insert bulk messages" ON public.bulk_messages
  ...
  CREATE POLICY "Users can view bulk messages" ON public.bulk_messages
    FOR SELECT TO authenticated USING (
      ...
      (EXISTS (SELECT 1 FROM public.users WHERE users.id = auth.uid() AND 'coordenador' = ANY(users.roles)))
    );
END $;

CREATE OR REPLACE FUNCTION public.send_internal_communication(
  p_target_roles text[],
  p_target_users uuid[],
  p_title text,
  p_message text,
